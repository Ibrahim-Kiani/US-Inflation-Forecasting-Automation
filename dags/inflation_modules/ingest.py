...
def fetch_fred(series_id, filename, start_date='1900-01-01'):
    ...
    if not is_monthly:
        ...
        # Using 'M' instead of 'ME' for better compatibility
    ...
    # Special handling for oil data to ensure it's in monthly format
    if series_id == 'DCOILWTICO':
        print(f"Ensuring oil data (DCOILWTICO) is in monthly format")
        # Double-check that it's properly resampled to monthly
        if not is_monthly:
            # Make sure we have a DatetimeIndex
            if not isinstance(df.index, pd.DatetimeIndex):
                df.index = pd.to_datetime(df.index)
            # Force monthly resampling for oil data
            df = df.resample('M').last()
            print(f"Forced monthly resampling for oil data: {len(df)} rows")
